...
func TestNewSynthesizerLogsStagesWithoutText(t *testing.T) {
	...
	const annotated = "(轻声)private-annotated-text"
	var providerText string
	var output bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&output, nil))
	synthesize := newSynthesizer(
		func(_ context.Context, text, _ string, _ int) ([]byte, error) {
			providerText = text
			return []byte("audio"), nil
		},
		func(context.Context, string) (string, error) { return annotated, nil },
		logger,
	)
	if _, err := synthesize(context.Background(), original, "冰糖", 25); err != nil {
		t.Fatal(err)
	}
	if providerText != annotated {
	...
	if strings.Contains(logs, original) || strings.Contains(logs, annotated) {
