...
默认 `EMOTION_RESPONSE_FORMAT=false`，只通过提示词要求索引 JSON，并继续在本地严格校验。这对不同 OpenAI 兼容服务更稳妥。如果你已经确认文字模型接口支持严格 JSON Schema，可显式设为 `true`，服务会发送 `response_format.type=json_schema`。

从旧版本升级时，现有 `.env` 不会随 `.env.example` 自动变化；请手动把 `EMOTION_RESPONSE_FORMAT` 改为 `false` 后重新创建容器。只拉取新镜像而保留旧值，仍会继续发送 `json_schema`。
...
该文件以追加方式写入，每行包含时间、请求 ID、成功/失败状态、稳定的 `error_category`、尝试次数、耗时以及文字模型的最终 `content`；校验成功时还包含生成的 `style_instruction`。提供方返回非 2xx 时，还会尽量记录状态码 `provider_status`，以及标准 `error.message` 中截断后的 `provider_error`，但不会复制非结构化原始错误页。新文件权限设为 `0600`，路径不可写时服务拒绝启动。

常见失败分类包括 `timeout`、`cancelled`、`provider_status`、`response_too_large`、`provider_json`、`content_json`、`invalid_range` 和 `invalid_style`。如果主要是 `timeout`，依据实测 P95 小幅提高 `EMOTION_TIMEOUT`；如果 `provider_status` 的 `provider_error` 表示不支持 `json_schema`，保持 `EMOTION_RESPONSE_FORMAT=false`；`invalid_range` 或 `invalid_style` 表示模型输出已收到但没有通过本地安全校验。
